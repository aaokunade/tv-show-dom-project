...
function getShows(allShows) {
  ...
  function selectEpisode(episodes) {
    ...
    episodes.forEach(function (episode) {
      const optionElem = document.createElement("option");
      optionElem.textContent = `S${String(episode["season"]).padStart(
        2,
        "0"
  ...
  showDisplayPara.style.display = "inline";
  fetchEpisodesPromise(showID).then(function (data) {
    selectEpisode(data);
  });
  ...
  function showInput(event) {
    ...
    showRemains = allShows.filter(function (showFiltered) {
      if (
        showFiltered["name"].toLowerCase().includes(search) ||
        showFiltered["summary"].toLowerCase().includes(search) ||
        showFiltered["genres"].join(" ").toLowerCase().includes(search)
      ) {
        return showFiltered;
      }
    });
  ...
  function displayEpisodes(episodes) {
    ...
    episodes.forEach(function (episode) {
      ...
      episodeDiv.id = `S${String(episode["season"]).padStart(2, "0")}E${String(
        episode["number"]
      ).padStart(2, "0")}`;
      const episodeHeader = document.createElement("h3");
      episodeHeader.innerHTML = `${episode["name"]} - S${String(
        episode["season"]
  ...
  function episodeInput(event) {
    ...
    fetchEpisodesPromise(showID).then(function (data) {
      listRemains = data.filter(function (episodeFiltered) {
        if (
          episodeFiltered["name"].toLowerCase().includes(search) ||
          episodeFiltered["summary"].toLowerCase().includes(search)
        ) {
